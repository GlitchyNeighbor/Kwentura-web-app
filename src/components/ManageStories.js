 ...
 * @param {boolean} [options.useOcrFallback=false] - Whether to use OCR when no text is found
 * @returns {Promise<string[]>} - Array of page texts
 */
export async function extractPdfPageTexts(pdfFile, options = {}) {
  const { useOcrFallback = false } = options;
...
const generateQuestionTTS = async (text, languageCode, storyId, type, index) => {
    ...
    try {
        const result = await synthesizeSpeechGoogle({ text, languageCode });

        if (!result.data || !result.data.audioData) {
            throw new Error("Invalid audio data from TTS function.");
        }
...
const ManageStories = () => {
  ...
  const validateForm = () => {
    ...
    if (!isEditing && !formData.pdfFile) {
      errors.pdfFile = "A PDF file is required for a new story.";
    }
  ...
  const handlePdfUpload = async (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      const baseMoralLesson = (isEditing && selectedStory && selectedStory.moralLesson)
        ? selectedStory.moralLesson
        : { question: "", options: ["", "", ""], correctOptionIndex: null };

      showAlert("Processing PDF, attempting to generate moral lesson...", "info");
      ...
      try {
        ...
        let finalMoralLesson = baseMoralLesson;
        ...
        if (extractedText) {
          try {
            ...
            const aiMoralLesson = moralLessonResult.data;
            if (aiMoralLesson.question && Array.isArray(aiMoralLesson.options) && aiMoralLesson.options.length === 3 && typeof aiMoralLesson.correctOptionIndex === 'number') {
              finalMoralLesson = aiMoralLesson;
              showAlert("Moral lesson question and options generated!", "success");
            } else {
              showAlert("Generated moral lesson from AI is not in the expected format.", "warning");
            }

            const aiQuestions = questionsResult.data.questions;
            if (
              ...
              finalComprehensionQuestions = aiQuestions;
              showAlert("Comprehension questions generated!", "success");
            } else {
              showAlert("Generated questions from AI are not in the expected format.", "warning");
            }
          } catch (aiError) {
            console.error("Error generating moral lesson or questions via Firebase Function:", aiError);
            showAlert(`Failed to generate moral lesson or questions: ${aiError.message}.`, "warning");
          }
        } else {
          showAlert("No text could be extracted from the PDF. Moral lesson not generated.", "warning");
        }
        ...
        setFormData(prevFormData => ({
          ...
          moralOptions: Array.isArray(finalMoralLesson?.options) && finalMoralLesson.options.length === 3 ? finalMoralLesson.options : ["", "", ""],
          moralCorrectOptionIndex: finalMoralLesson?.correctOptionIndex !== undefined ? finalMoralLesson.correctOptionIndex : null,
          comprehensionQuestions: finalComprehensionQuestions,
        }));

      } catch (error) {
        console.error("Error processing PDF or generating moral lesson:", error);
        showAlert(`Failed to process PDF or generate moral lesson: ${error.message}. You can still upload the file.`, "danger");
      } finally {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (finalPdfUrl && randomStoryId) {
      ...
      await generateStorySynopsis({ storyId: randomStoryId })
        .then(() => {
          showAlert("Synopsis generation process completed. It will appear in the story details soon.", "success");
        })
        .catch((error) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      showAlert("Story and all related student progress/bookmarks deleted successfully!", "success");
      fetchStoriesFromFirestore();

    } catch (error) {
      showAlert("Error deleting story and related data", "danger");
          ...
          <Row className="w-100">
            ...
            <Col>
              <Button
                ...
                disabled={isSubmitting || (!isEditing && !formData.pdfFile)}
                style={{
                  backgroundColor: COLORS.pink,
                  border: "none",
                  borderRadius: "25px",
