  ...
  const profileTooltip = (
    <Tooltip id="settings-profile-tooltip" className="custom-profile-tooltip">
      <div className="text-start p-2">
        <div className="fw-semibold mb-1" style={{ fontSize: '0.9rem' }}>{userName || 'Teacher'}</div>
        {userRole && (
          <div className="text-light small">
            <strong>Role:</strong> {userRole.replace(/([A-Z])/g, ' $1').trim()}
          </div>
        )}
      </div>
    </Tooltip>
              ...
              <OverlayTrigger
                ...
                overlay={
                  <Tooltip id="collapsed-profile-tooltip">
                    <div className="text-start p-2">
                      <div className="fw-semibold mb-1">{userName || 'Teacher'}</div>
                      {userRole && (
                        <div className="text-light small">
                          <strong>Role:</strong> {userRole.replace(/([A-Z])/g, ' $1').trim()}
                        </div>
...
export default React.memo(SidebarSettingsTeacher);